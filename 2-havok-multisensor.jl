...
fig_savepath = joinpath(figpath, "2-havok-multisensor")
...
function eval_havok(Zs, ts, n_embedding, r_model, n_control; method=:backward)
    ...
    idx_int = 1:(7*24*60)  # three days total
    ts_int = ts_x[idx_int]
...
save(joinpath(fig_savepath, "havok-rmse-1-hour.png"), fig)
save(joinpath(fig_savepath, "havok-rmse-1-hour.pdf"), fig)
...
save(joinpath(fig_savepath, "havok-rmse-12-hour.png"), fig)
save(joinpath(fig_savepath, "havok-rmse-12-hour.pdf"), fig)
...
save(joinpath(fig_savepath, "havok-rmse-24-hour.png"), fig)
save(joinpath(fig_savepath, "havok-rmse-24-hour.pdf"), fig)
...
save(joinpath(fig_savepath, "havok-rmse-48-hour.png"), fig)
save(joinpath(fig_savepath, "havok-rmse-48-hour.pdf"), fig)
...
save(joinpath(fig_savepath, "havok-rmse-72-hour.png"), fig)
save(joinpath(fig_savepath, "havok-rmse-72-hour.pdf"), fig)
...
l_orig = lines!(ax, ts_x[1:3*24*60], Zs_x[1:3*24*60], linewidth=3)
l_havok = lines!(ax, ts_x[1:3*24*60], Ẑs_x[1:3*24*60], linewidth=2)
...
# can we get a fixed time step solver to work as well? 

# train NN to learn the forcing function
