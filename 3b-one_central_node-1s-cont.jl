...
Ltest = L[end]+1:length(ts_x)
@assert length(L) + length(Ltest) == length(ts_x)
...
xs = zeros(length(ts_x[L]), n_control)
xs_test = zeros(length(ts_x[Ltest]), n_control)
for i ∈ axes(xs,1)
    xs[i,:] .= u(ts_x[i])
end

for i ∈ axes(xs_test,1)
    xs_test[i,:] .= u(ts_x[Ltest[i]])
end
...
l1 = lines!(ax, ts_x[1:Nmax] ./ (60^2*24), X[1:Nmax,1], linewidth=3)
l2 = lines!(ax2, ts_x[1:Nmax] ./ (60^2*24), xs[1:Nmax,1].^2, linewidth=3, color=mints_colors[2])

rowsize!(gl, 2, Relative(0.2));
xlims!(ax2, ts_x[1] ./ (60^2*24), ts_x[Nmax] ./ (60^2*24))
fig
...
prob = ODEProblem(f!, x₀, (ts_x[L[1]], ts_x[L[end]]), params);
sol = solve(prob, saveat=ts_x[L]);
size(sol)
...
prob_test = ODEProblem(f!, x₀, (ts_x[Ltest[1]], ts_x[Ltest[end]]), params)
sol_test = solve(prob_test, saveat=ts_x[Ltest]);
size(sol_test)
...
l1 = lines!(ax, ts_x[1:Nmax] ./ (60^2), X[1:Nmax,1], linewidth=2)
l2 = lines!(ax, ts_x[1:Nmax] ./ (60^2), X̂[1:Nmax,1], linewidth=2, alpha=0.75, color=mints_colors[2])

l1 = lines!(ax2, ts_x[1:Nmax] ./ (60^2), X[1:Nmax,2], linewidth=2)
l2 = lines!(ax2, ts_x[1:Nmax] ./ (60^2), X̂[1:Nmax,2], linewidth=2, alpha=0.75, color=mints_colors[2])

l1 = lines!(ax3, ts_x[1:Nmax] ./ (60^2), X[1:Nmax,3], linewidth=2)
l2 = lines!(ax3, ts_x[1:Nmax] ./ (60^2), X̂[1:Nmax,3], linewidth=2, alpha=0.75, color=mints_colors[2])
