...
# idx_train_short = 1:(3*60)
idx_train_short = 1:30
...
const A = Ξ[:, 1:r-n_control]   # State matrix A
const B = Ξ[:, r-n_control+1:end]      # Control matrix B
...
function loss(θ)
    X̂ = predict(θ)
    # Z_pred = (U_small[1,:]' * diagm(σ_small) * X̂)'
    # mean(abs2, Z_pred .- Z_orig)
    mean(abs2, X̂ .- X[idx_train_short, 1:r-n_control]')
end
...
res1 = Optimization.solve(optprob, ADAM(), callback=callback, maxiters=7500)
...
fig = Figure();
ax = CairoMakie.Axis(fig[1,1])
lines!(ax, Z_orig[1:100], label="Orig")

lines!(ax, Ẑ[1:100], label="HAVOK")
fig
