...
fig_savepath = joinpath(fig_savepaths[f_to_use], "df-9-day-no-interp")
...
for i ∈ 1:length(Xs)
    ...
    l1 = lines!(ax2, ts_x[i] ./ (60^2), Xs[i][:,2], linewidth=2, color=mints_colors[1])
    l2 = lines!(ax2, ts_x[i] ./ (60^2), X̂s[i][:,2], linewidth=2, alpha=0.75, color=mints_colors[2])
